...
### Chosen Approach
I chose a rule-based approach because this task focuses on structured thinking and handling inconsistent product data in a clear and controlled way.  
The solution is based on text normalization, Hebrew-English term mapping, and comparison of key product details.  
...
מטרת הפתרון היא לאחד רשומות המתייחסות לאותו מוצר, תוך הימנעות ממיזוג שגוי של מוצרים שונים, ובסיום להציג ללקוח את המחיר הנמוך ביותר מתוך כלל הרשומות ששויכו לאותו מוצר.

### הגישה שנבחרה 
בחרתי בגישה מבוססת חוקים, כי המשימה מתמקדת בחשיבה מסודרת ובהתמודדות עם מידע לא אחיד של מוצרים בצורה ברורה ומבוקרת.
הפתרון מבוסס על נרמול טקסט, מיפוי מונחים מקבילים בין עברית לאנגלית והשוואה בין מאפיינים מרכזיים של המוצר.
...
לאחר קיבוץ הרשומות הרלוונטיות, נבחר המחיר הנמוך ביותר מכל קבוצת המוצרים המאוחדת.
